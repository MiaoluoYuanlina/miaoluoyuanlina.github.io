...
# 将全部逻辑封装在函数中，防止直接复制粘贴时 read 命令导致语法错误
main() {
    # 设置遇到错误即暂停/停止
    set -e
    ...
    # 0. 磁盘空间清理 (针对 no space left on device 错误)
    echo -e "${GREEN}[0/8] 正在清理上次失败残留的临时文件...${NC}"
    rm -rf /var/tmp/podman* /var/tmp/container* /tmp/podman* || true

    echo -e "${YELLOW}您的磁盘空间可能不足，是否在迁移前清理无用的 Docker 数据(悬空镜像、已停止的容器)以释放空间? (y/n)${NC}"
    read -r clean_choice
    if [[ "$clean_choice" =~ ^[Yy]$ ]]; then
        if command -v docker >/dev/null 2>&1 && systemctl is-active --quiet docker; then
            echo -e "${GREEN}正在清理无用的 Docker 数据...${NC}"
        ...
        if command -v docker >/dev/null 2>&1 && systemctl is-active --quiet docker; then
            ...
            if [ -n "$IMAGES" ]; then
                for img in $IMAGES; do
                    echo "正在迁移镜像: $img"
                    # 增加容错和垃圾清理
                    if ! docker save "$img" | podman load; then
                        echo -e "${YELLOW}警告: 镜像 $img 迁移失败，可能是磁盘空间不足，跳过并清理临时文件...${NC}"
                        rm -rf /var/tmp/podman* /var/tmp/container* /tmp/podman* || true
                    fi
                done
            else
    ...
    if [ "$DOWNLOAD_SUCCESS" = false ]; then
        echo -e "${RED}严重错误: Docker Compose 下载失败，请检查网络。${NC}"
        exit 1
    fi
